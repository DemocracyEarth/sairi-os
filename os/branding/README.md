...
runs. The shell and `packages/ui-components` consume the same tokens, so there is one
source of truth rather than a design system that drifts from the login screen.
...
Nothing here has been rendered on a display, rasterised, or measured.
