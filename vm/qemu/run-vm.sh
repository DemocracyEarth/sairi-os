...
if [ "$FORWARD_SHELL" -eq 1 ]; then
	...
	say ""
	say "      ./vm/qemu/tunnel.sh"
	say "      open http://127.0.0.1:7800/#/os"
	say ""
	say "  Tunnel all FOUR ports, not just 7800. The shell calls 7801-7803 and"
	say "  its CORS allowlist names those exact origins, so forwarding the shell"
	say "  alone gives you a desktop where every request fails."
	say ""
